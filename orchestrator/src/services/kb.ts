...
export async function getKbContext(db: DbPool, input: KbQueryInput): Promise<KbContext> {
  ...
  const policiesRes = await db.query(
    `SELECT key, scope, phone, value_json, priority, is_enabled, description, updated_at, updated_by
     FROM agent_policies
  ...
  const playbooksRes = await db.query(
    `SELECT id, scenario_key, language, instruction, priority, is_enabled, tags, updated_at, updated_by
     ...
     LIMIT 5`,
    [intent, `${intent}_%`, JSON.stringify([intent])]
  );

  // Templates: top N by weight for intent+language.
  const templatesRes = await db.query(
    `SELECT id, name, intent, language, body, tags, is_enabled, weight, updated_at, updated_by
     ...
     LIMIT $3`,
    [intent, language, limits.templates]
  );

  // Examples GOOD
  const goodRes = await db.query(
    `SELECT id, intent, language, label, client_text, agent_text, explanation, tags, weight, is_enabled, created_at, updated_at, updated_by
     FROM agent_examples
     WHERE is_enabled = TRUE
       AND intent = $1
     ...
     LIMIT $3`,
    [intent, language, limits.goodExamples]
  );

  // Examples BAD
  const badRes = await db.query(
    `SELECT id, intent, language, label, client_text, agent_text, explanation, tags, weight, is_enabled, created_at, updated_at, updated_by
     FROM agent_examples
     WHERE is_enabled = TRUE
       AND intent = $1
     ...
     ORDER BY weight DESC, created_at DESC
     LIMIT $3`,
    [intent, language, limits.badExamples]
  );
...
export function buildKbContextBlock(kb: KbContext, maxChars: number): string {
  const lines: string[] = [];
  ...
  if (kb.policies.length) {
    ...
    for (const p of kb.policies.slice(0, maxPolicies)) {
      ...
      let valueStr: string;
      if (v === null || v === undefined) valueStr = 'null';
  ...
  // Sanitizer flags
  const cancelNeedsApproval = kb.policies.some(
    (p) =>
      p.key === 'cancellation.always_approval' &&
      (p.value_json === true || String(p.value_json).toLowerCase() === 'true')
  );

  const conflictMarkers = ['stornier', 'cancel', 'отмен'];

  function isConflictingText(text: string | undefined): boolean {
  ...
  if (cancelNeedsApproval) {
    bad = bad.filter((e) => {
      ...
      if (badText) conflictDropped = true;
      return !badText;
    });
  }
  if (bad.length) {

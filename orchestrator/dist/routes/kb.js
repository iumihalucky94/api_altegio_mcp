...
async function registerKbRoutes(app) {
    ...
    app.kbInternalToken = async () => (await app.getConfigString?.('KB_INTERNAL_TOKEN', '')) ||
        (await app.getConfigString?.('MCP_INTERNAL_TOKEN', '')) ||
        '';
    // GET /kb/context
    app.get('/kb/context', async (request, reply) => {
        ...
        if (ok === null)
            return;
        const intent = request.query?.intent || 'UNKNOWN';
        const language = request.query?.lang || 'de';
        const phone = request.query?.phone || null;
        const templatesLimit = Number((await app.getConfigNumber?.('KB_CONTEXT_LIMIT_TEMPLATES', 3)) || 3);
        const goodLimit = Number((await app.getConfigNumber?.('KB_CONTEXT_LIMIT_GOOD_EXAMPLES', 3)) || 3);
        const badLimit = Number((await app.getConfigNumber?.('KB_CONTEXT_LIMIT_BAD_EXAMPLES', 1)) || 1);
        const context = await (0, kb_1.getKbContext)(db, {
            intent,
            language,
            phone,
            messageText: request.query?.q || '',
            limits: {
    ...
    app.post('/kb/policies', async (request, reply) => {
        ...
        const res = await db.query(`INSERT INTO agent_policies (key, scope, phone, value_json, priority, is_enabled, description, updated_by)
    ...
    app.patch('/kb/policies', async (request, reply) => {
        ...
        const body = request.body;
        const updatedBy = getUpdatedBy(request);
        const res = await db.query(`UPDATE agent_policies
       SET
    ...
    app.delete('/kb/policies', async (request, reply) => {
       ...
       WHERE key = $1 AND scope = COALESCE($2, scope) AND (phone = COALESCE($3, phone) OR (phone IS NULL AND $3 IS NULL))
       RETURNING key, scope, phone, value_json, priority, is_enabled, description, updated_at, updated_by`, [body.key, body.scope || null, body.phone || null, updatedBy]);
        if (res.rows.length === 0)
            return reply.code(404).send({ error: 'Policy not found' });
        const row = res.rows[0];
        await (0, audit_1.logAudit)(db, {
    ...
    app.post('/kb/templates', async (request, reply) => {
        ...
        const body = request.body;
        const updatedBy = getUpdatedBy(request);
        const res = await db.query(`INSERT INTO agent_templates (name, intent, language, body, tags, is_enabled, weight, updated_by)
    ...
    app.patch('/kb/templates/:id', async (request, reply) => {
        ...
        const body = request.body;
        const updatedBy = getUpdatedBy(request);
        const res = await db.query(`UPDATE agent_templates
       SET
    ...
    app.post('/kb/examples', async (request, reply) => {
        ...
        const body = request.body;
        const updatedBy = getUpdatedBy(request);
        const res = await db.query(`INSERT INTO agent_examples (intent, language, label, client_text, agent_text, explanation, tags, weight, is_enabled, updated_by)
    ...
    app.patch('/kb/examples/:id', async (request, reply) => {
        ...
        const body = request.body;
        const updatedBy = getUpdatedBy(request);
        const res = await db.query(`UPDATE agent_examples
       SET
    ...
    app.post('/kb/playbooks', async (request, reply) => {
        ...
        const body = request.body;
        const updatedBy = getUpdatedBy(request);
        const res = await db.query(`INSERT INTO agent_playbooks (scenario_key, language, instruction, priority, is_enabled, tags, updated_by)
    ...
    app.patch('/kb/playbooks/:id', async (request, reply) => {
        ...
        const body = request.body;
        const updatedBy = getUpdatedBy(request);
        const res = await db.query(`UPDATE agent_playbooks
       SET

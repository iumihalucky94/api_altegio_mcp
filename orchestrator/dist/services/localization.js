...
exports.resolveReplyLanguage = resolveReplyLanguage;
exports.effectiveLangForReply = effectiveLangForReply;
exports.getSystemMessage = getSystemMessage;
const VALID_RESOLVED = ['de', 'ru', 'en'];
const MESSAGES = {
    booking_failed: {
 ...
 */
function resolveReplyLanguage(batchText, languageHint, languagePreference) {
    const hint = (languagePreference ?? languageHint ?? '').trim().toLowerCase();
    if (hint === 'de' || hint === 'ru' || hint === 'en')
        return hint;
    ...
    if (hasCyrillic && (hasRussianWords || !hasLatin(t)))
        return 'ru';
    if (hasGermanUmlaut || hasGermanWords)
        return 'de';
    if (hasEnglishWords && hasLatin(t))
        return 'en';
    if (hasCyrillic)
        return 'ru';
    if (hasLatin(t))
 ...
 */
function effectiveLangForReply(lang, batchText, languageHint, languagePreference) {
    if (lang !== 'mixed' && VALID_RESOLVED.includes(lang)) {
        return lang;
    }
 ...
 */
function getSystemMessage(key, lang, vars) {

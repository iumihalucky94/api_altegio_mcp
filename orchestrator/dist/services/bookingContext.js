...
exports.matchStaffFromMessage = matchStaffFromMessage;
exports.getDatesToFetch = getDatesToFetch;
 ...
 */
function resolveRelativeDate(text, timezone = TZ_DEFAULT) {
    ...
    let targetDow = matchWeekday(weekdaysRu) ?? matchWeekday(weekdaysEn) ?? matchWeekday(weekdaysDe);
    ...
    if (targetDow == null) {
        const ruAccusative = { понедельник: 1, вторник: 2, среду: 3, четверг: 4, пятницу: 5, субботу: 6, воскресенье: 0 };
...
function transliterateCyrToLat(s) {
 ...
 */
function matchStaffFromMessage(text, staff) {
    if (!text || !staff.length)
        return null;
    ...
    for (const s of staff) {
        ...
        if (lower.includes(nameLat) || lower.includes(name) || lowerCyrLat.includes(nameLat) || lowerCyrLat.includes(nameCyrLat))
            return s.id;
        const nameWords = name.split(/\s+/).filter((w) => w.length >= 2);
        for (const part of nameWords) {
            const partLat = part.toLowerCase();
            const partCyrLat = transliterateCyrToLat(part);
            if (lower.includes(partLat) || lower.includes(part) || lowerCyrLat.includes(partLat) || lowerCyrLat.includes(partCyrLat))
                return s.id;
        }
    }
    ...
    if (m) {
        ...
        if (namePart.length >= 2) {
            ...
            for (const s of staff) {
                ...
                if (name.includes(namePart) || namePart.includes(name) || nameCyrLat.includes(namePartLat) || namePartLat.includes(nameCyrLat))
                    return s.id;
            }
        }
    }
